...
_turtle = ((nearestObjects[getPos player,["Hen_random_F","Cock_random_F","Goat_random_F","Sheep_random_F","Rabbit_F","Turtle_F"],3]) select 0);
...
if(isPlayer _curTarget && _curTarget isKindOf "Man") then {
	if( ((_curTarget getVariable["restrained", false]) && !dialog ) || playerside == independent ) then {
		switch( playerSide ) do {
			...
			case east: {};
			case independent: {[_curTarget] call life_fnc_medicInteractionMenu;};
